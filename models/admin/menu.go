...
type Menu struct {
	...
	Title  string `orm:"column(title);size(255);null" description:"栏目名称"`
	Target string `orm:"column(target);size(255);null" description:"链接打开方式"`
	Url    string `orm:"column(url);size(255);null" description:"链接URL"`
	Sort   int    `orm:"column(sort)" description:"排序"`
	Pid    int    `orm:"column(pid);null" description:"顶级栏目"`
}
